...
interface IOutputFormatter {
    public function toString(Invoice $invoice);
}
abstract class OutputFormatter implements IOutputFormatter
{
    public $Quiz;
    abstract public function toString(Invoice $invoice);
}
class JsonOutputFormatter extends OutputFormatter implements IOutputFormatter
{
    public function toString(Invoice $invoice)
    {
        ...
        $questions = $this->Invoice->getPositions();
        $json = new stdClass();
...
class Quiz
{
    ...
    */      //Working: \\n\s*([1-9]{1}\d{0,2})\s*(?=\\t+)\s*(.+?)\\t\s?\\n\W
    ...
    private $skipPages = 6; // how many pages to skip; most have two
    ...
    private function parseAnswers($input)
    {
        ...
        // first input question A should start as " \nA ..." to match the others
        $input = " \n" .$input;
        //$answers = preg_split("#(\u{f0a8}|\u{f0fe})#umi", $input, -1, PREG_SPLIT_DELIM_CAPTURE);
        $answers = preg_split("#(\s?\n[A-D]{1}\s+)#", $input, -1, PREG_SPLIT_DELIM_CAPTURE);
    ...
    private function parseQuestions()
    {
        ...
        if ($this->numberOfQuestions * 3 !== $splitCount) :
            throw new Exception("Unexpected number of text splits $this->numberOfQuestions * 3 !== $splitCount");
        endif;
        ...
        foreach ($chunks as $question) :
            ...
            try {
                $answers = $this->parseAnswers($question[2]);
            } catch (Exception $ex) {
...
class quizParser
{
    ...
    public function parseDirectory()
    {
        ...
        foreach ($directory as $file) {
            ...
            $quiz = new Invoice($file->getPathname(), $this->pdfParser);
            try {
                $this->quizes[] = $quiz;
            } catch (Exception $ex) {
...
/*
$quiz = new Invoice('./pdf/AZF/Pruefungsfragen_AZF.pdf', $parser);
header('Content-Type: application/json');
echo $quiz->getJson();
die();
*/

$quizParser = new invoiceParser('./pdf/AZF/', $parser);
